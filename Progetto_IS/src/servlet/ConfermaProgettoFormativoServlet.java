...
public class ConfermaProgettoFormativoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(tipoAccount == 1){
			...
			try {
				UfficioDCS.confermaPf(idProgettoFormativo);
				pagina="VisualizzazioneProgettiFormativiServlet"; //in questo caso richiamiamo la servlet
				session.setAttribute("flagConferma", 1);
			} catch (ClassNotFoundException | SQLException e) {
				System.out.println("Errore nella conferma del ProgettoFormativo. idProgettoFormativo-> ["+idProgettoFormativo+" ]");
				e.printStackTrace();
			}
